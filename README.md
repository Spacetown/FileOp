...
For development a workspace for `Visual Studio Code` is configured together with a devcontainer.
Inside the devcontainer the needed tools for cross compiling are installed by
[script/container.setup.sh](./scripts/container.setup.sh) which is executed after container creation.
