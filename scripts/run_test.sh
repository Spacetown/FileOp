...
(
   ...
   ./FileOp.exe --help | tee stdout.log | sed 's/^/   stdout: /'
   ...
   grep -F "FileOp.exe: error: Unknown command 'unknown' given, use option --help for more information." stderr.log > /dev/null
   echo " -> Found expected output in log"
   rm -f stderr.log
) || exit 1
