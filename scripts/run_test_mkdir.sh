...
echo "Run help..."
FileOp.exe mkdir --help | sed 's/^/   stdout: /'

echo "Use unknown option..."
if FileOp.exe mkdir --xxx 2> stderr.log | sed 's/^/   stdout: /' ; then
...
if FileOp.exe mkdir 2> stderr.log | sed 's/^/   stdout: /' ; then
    cat stderr.log | sed 's/^/   stderr: /'
    echo " -> Got unexpected exit code 0."
    exit 1
else
...
if FileOp.exe --debug mkdir -- --help | sed 's/^/   stdout: /'; then
    echo " -> Got expected exit code 0."
else
    echo " -> Got unexpected error."
    exit 1
