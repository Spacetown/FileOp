...
echo "Run help..."
FileOp.exe move --help | sed 's/^/   stdout: /'

echo "Use unknown option..."
if FileOp.exe move --xxx 2> stderr.log | sed 's/^/   stdout: /' ; then
...
echo "move with missing argument..."
if FileOp.exe move 2> stderr.log | sed 's/^/   stdout: /' ; then
    cat stderr.log | sed 's/^/   stderr: /'
    echo " -> Got unexpected exit code 0."
    exit 1
else
...
grep -E "FileOp.exe: error: Can't move file .+\\\\$$\\\\file to .+\\\\$$\\\\file2: The system cannot find the path specified." stderr.log > /dev/null
echo " -> Found expected output in log"
rm -f stderr.log
...
if FileOp.exe copy ${BASH_SOURCE[0]}.tmp /tmp/$$/test/ 2> stderr.log | sed 's/^/   stdout: /' ; then
    cat stderr.log | sed 's/^/   stderr: /'
    echo " -> Got unexpected exit code 0."
    exit 1
else
...
echo "Test" > "--source"
if FileOp.exe --debug move -- --source --target | sed 's/^/   stdout: /' ; then
...
if FileOp.exe --debug move --check-unique-names --target-directory /tmp/$$ ${BASH_SOURCE[0]} "/tmp/$$/script/$(basename ${BASH_SOURCE[0]})" 2> stderr.log | sed 's/^/   stdout: /' ; then
    cat stderr.log | sed 's/^/   stderr: /'
    echo " -> Got unexpected exit code 0."
    exit 1
else
...
cp -rf $(dirname ${BASH_SOURCE[0]})/*.* /tmp/$$/source/
if FileOp.exe --debug move --force /tmp/$$/source/*.* /tmp/$$ | sed 's/^/   stdout: /' ; then
    echo " -> Got expected exit code 0."
else
    echo " -> Got unexpected error."
    exit 1
...
if FileOp.exe --debug move --touch --force --time $Timestamp --target-directory /tmp/$$ /tmp/$$/source/scripts | sed 's/^/   stdout: /' ; then
    echo " -> Got expected exit code 0."
else
    echo " -> Got unexpected error."
    exit 1
