...
tBool clearReadonly(LPCTSTR currentPath, const DWORD dwAttrs)
{
   if (Debug)
   {
      print(_T("Clear readonly flag of element [%s]\n"), currentPath);
   }
...
tBool removeReparsePoint(LPCTSTR currentPath)
{
   if (Debug)
   {
      print(_T("Remove reparse point [%s]\n"), currentPath);
   }
   ...
   else
   {
      ...
      if ( pft == NULL ) {
         ...
         if ( localTime == NULL ) {
            ...
            if (count < 3) {
               return printError(_T("Wrong format for time [%s], expected [%s].\n"), localTime, TIME_FORMAT);
            }
            if ( count == 3 ) {
               lt.wHour = 12;
               lt.wMinute = 0;
...
tBool runCommandForEachInputLine(int argc, wchar_t *argv[], tBool (*command)( void ))
{
   ...
   while ((result == VALUE_TRUE) && (argc-- != 0))
   {
      ...
      else
      {
         ...
         {
            result &= printError(_T("Argument must not be empty.\n" ));
         }
         else
         {
            result &= command();
...
tBool checkUniqueName(void)
{
   ...
   while(*CurrentFilename != NULL) {
      if (_tcscmp(PtrFilenameStart, *CurrentFilename) == 0) {
         result &= printError(_T("File in source list will overwrite each other: [%s]"), PtrFilenameStart);
         break;
      }
      ++CurrentFilename;
   }
