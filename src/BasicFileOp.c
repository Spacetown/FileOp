 ...
 * @return eTrue if file attributes are valid
 */
tBool isValidFileAttributes(const DWORD dwAttrs) {
   return (dwAttrs == INVALID_FILE_ATTRIBUTES) ? eFalse : eTrue;
}
 ...
 *
 * @param dwAttrs The attributes to check.
 * @return eTrue if readonly attribute is set, else eFalse.
 */
tBool isReadonly(const DWORD dwAttrs) {
   return (isValidFileAttributes(dwAttrs) && ((dwAttrs & FILE_ATTRIBUTE_READONLY) != 0)) ? eTrue : eFalse;
}
 ...
 *
 * @param dwAttrs The attributes to check.
 * @return eTrue if element is a reparse point, else eFalse.
 */
tBool isReparsePoint(const DWORD dwAttrs) {
   return (isValidFileAttributes(dwAttrs) && ((dwAttrs & FILE_ATTRIBUTE_REPARSE_POINT) != 0)) ? eTrue : eFalse;
}
 ...
 *
 * @param dwAttrs The attributes to check.
 * @return eTrue if directory attribute is set, else eFalse.
 */
tBool isDirectory(const DWORD dwAttrs) {
   return (isValidFileAttributes(dwAttrs) && ((dwAttrs & FILE_ATTRIBUTE_DIRECTORY) != 0)) ? eTrue : eFalse;
}
 ...
 * @param createIfMissing If eTrue, the file is created if it doesn't exist.
 *
 * @return eTrue if the file was successfully touched, else eFalse.
 */
 ...
 *
 * @return eTrue if the file was opened successfully, also setting argument handle, else eFalse.
 */
      ...
      if (filename[0] == _T('@')) {
         ...
         if (result == eTrue) {
            ...
            *IoBufferWideCharacter = _T('\0');
            ...
            } while ((result == eTrue) && (BytesRead != 0));
 ...
 *
 * @return eTrue if everything is unique, else eFalse
 */
