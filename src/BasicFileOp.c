...
tResult clearReadonly(LPCTSTR currentPath, const DWORD dwAttrs) {
   ...
   if (SetFileAttributes(currentPath, dwAttrs & (~FILE_ATTRIBUTE_READONLY)) == 0) {
      return printLastError(_T("Can't clear readonly flag of element %s"), &currentPath[DOS_DEVICE_PREFIX_LENGTH]); // GCOVR_EXCL_LINE
   }
   return eOk;
}
...
tResult removeReparsePoint(LPCTSTR currentPath) {
   ...
   if (Handle == INVALID_HANDLE_VALUE) {
      return printLastError(_T("Can't get handle to %s"), &currentPath[DOS_DEVICE_PREFIX_LENGTH]); // GCOVR_EXCL_LINE
   }
   ...
   if (DeviceIoControl(
      Handle, FSCTL_DELETE_REPARSE_POINT, &ReparseDataBuffer, REPARSE_DATA_BUFFER_HEADER_SIZE, NULL, 0,
      &BytesReturned, 0) == 0
   ) {
      // GCOVR_EXCL_START
   ...
   if (CloseHandle(Handle) == 0) {
      return printLastError(_T("Can't close handle to %s"), &currentPath[DOS_DEVICE_PREFIX_LENGTH]); // GCOVR_EXCL_LINE
   }
 ...
 */
tResult touchSingleFile(LPCTSTR currentPath, LPCTSTR localTime, tBool createIfMissing) {
   HANDLE hFile;
   ...
   if (hFile == INVALID_HANDLE_VALUE) {
      ...
      if (SetFileTime(hFile, (LPFILETIME) NULL, pft, pft) == 0) {
         return printLastError(_T("Can't set access and write time of %s"), &currentPath[DOS_DEVICE_PREFIX_LENGTH]); // GCOVR_EXCL_LINE
      }
   ...
   if (result == eOk) {
      ...
      do {
         if (FALSE == ReadFile(InHandle, IoBuffer, SIZE_IO_BUFFER, &BytesRead, NULL)) {
            result &= printLastError(_T("Error reading file %s"), &currentPath[DOS_DEVICE_PREFIX_LENGTH]); // GCOVR_EXCL_LINE
         }
         if (BytesRead != 0) {
