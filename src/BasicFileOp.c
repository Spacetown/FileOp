...
const int DOS_DEVICE_PREFIX_LENGTH =
    sizeof(DosDevicePathPrefix) / sizeof(TCHAR);
LPCTSTR DosDevicePathPrefixUnc = _T("\\\\?\\UNC\\");
// The length of the DOS device path prefix for UNC paths
const int DOS_DEVICE_PREFIX_UNC_LENGTH =
    sizeof(DosDevicePathPrefixUnc) / sizeof(TCHAR);
...
void createDosDevicePath(LPCTSTR currentSourcePath, LPTSTR currentPath) {
  ...
  DWORD Size = GetFullPathName(currentSourcePath, DOS_DEVICE_BUFFER_SIZE,
                               TempSourcePath, NULL);
  // GCOVR_EXCL_START
  if (Size == 0L) {
    ...
    printLastError(_T("Could not get full name of %s. Buffer is to small, ")
                   _T("needed are %d characters."),
  ...
  if (_tcsncmp(StartOfPath, DosDevicePathPrefix, DOS_DEVICE_PREFIX_LENGTH) ==
      0) {
    currentPath[0] = _T('\0');
  } else {
...
LPCTSTR getReadableFilename(LPCTSTR currentPath) {
  return &currentPath[DOS_DEVICE_PREFIX_LENGTH];
}
...
tResult isReadonly(const DWORD dwAttrs) {
  return (isValidFileAttributes(dwAttrs) &&
          ((dwAttrs & FILE_ATTRIBUTE_READONLY) != 0))
             ? eOk
             : eError;
}
...
tResult isReparsePoint(const DWORD dwAttrs) {
  return (isValidFileAttributes(dwAttrs) &&
          ((dwAttrs & FILE_ATTRIBUTE_REPARSE_POINT) != 0))
             ? eOk
             : eError;
}
...
tResult clearReadonly(LPCTSTR currentPath, const DWORD dwAttrs) {
  ...
  if (SetFileAttributes(currentPath, dwAttrs & (~FILE_ATTRIBUTE_READONLY)) ==
      0) {
    return printLastError(_T("Can't clear readonly flag of element %s"),
                          getReadableFilename(currentPath)); // GCOVR_EXCL_LINE
  }
  return eOk;
}
...
tResult createSingleDirectory(LPCTSTR currentPath) {
  ...
  if (CreateDirectory(currentPath, NULL) == 0) {
    return printLastError(_T("Can't create directory %s"),
                          getReadableFilename(currentPath));
  }
...
tResult removeEmptyDirectory(LPCTSTR currentPath) {
  ...
  if (RemoveDirectory(currentPath) == 0) {
    return printLastError(_T("Can't remove directory %s"),
                          getReadableFilename(currentPath));
  }
...
tResult removeReparsePoint(LPCTSTR currentPath) {
  ...
  HANDLE Handle = CreateFile(
      currentPath, GENERIC_READ | GENERIC_WRITE, 0, NULL, OPEN_EXISTING,
      FILE_FLAG_BACKUP_SEMANTICS | FILE_FLAG_OPEN_REPARSE_POINT, NULL);
  if (Handle == INVALID_HANDLE_VALUE) {
    return printLastError(_T("Can't get handle to %s"),
                          getReadableFilename(currentPath)); // GCOVR_EXCL_LINE
  }
  ...
  if (DeviceIoControl(Handle, FSCTL_DELETE_REPARSE_POINT, &ReparseDataBuffer,
                      REPARSE_DATA_BUFFER_HEADER_SIZE, NULL, 0, &BytesReturned,
                      0) == 0) {
    // GCOVR_EXCL_START
    printLastError(_T("Can't remove reparse point %s"),
                   getReadableFilename(currentPath));
    if (CloseHandle(Handle) == 0) {
      printLastError(_T("Can't close handle to %s"),
                     getReadableFilename(currentPath));
    }
    return eError;
    // GCOVR_EXCL_STOP
  ...
  if (CloseHandle(Handle) == 0) {
    return printLastError(_T("Can't close handle to %s"),
                          getReadableFilename(currentPath)); // GCOVR_EXCL_LINE
  }
...
tResult removeSingleFile(LPCTSTR currentPath) {
  ...
  if (DeleteFile(currentPath) == 0) {
    return printLastError(_T("Can't remove file %s"),
                          getReadableFilename(currentPath));
  }
 ...
 * @return eOk on success, else eError.
 */
tResult copySingleFile(LPCTSTR currentSourcePath, LPCTSTR currentTargetPath,
                       tBool force) {
  if (Debug) {
    printOut(_T("Copy file %s to %s\n"), getReadableFilename(currentSourcePath),
             getReadableFilename(currentTargetPath));
  }
  if (CopyFile(currentSourcePath, currentTargetPath, force ? 0 : 1) == 0) {
    return printLastError(_T("Can't copy file %s to %s"),
                          getReadableFilename(currentSourcePath),
                          getReadableFilename(currentTargetPath));
  }
 ...
 * @return eOk on success, else eError.
 */
tResult moveSingleFile(LPCTSTR currentSourcePath, LPCTSTR currentTargetPath,
                       tBool force) {
  if (Debug) {
    printOut(_T("Move file %s to %s\n"), getReadableFilename(currentSourcePath),
             getReadableFilename(currentTargetPath));
  }
  if (MoveFileEx(currentSourcePath, currentTargetPath,
                 MOVEFILE_COPY_ALLOWED |
                     (force ? MOVEFILE_REPLACE_EXISTING : 0)) == 0) {
    return printLastError(_T("Can't move file %s to %s"),
                          getReadableFilename(currentSourcePath),
                          getReadableFilename(currentTargetPath));
  }
...
tResult touchSingleFile(LPCTSTR currentPath, tBool createIfMissing) {
  ...
  hFile = CreateFile(currentPath, FILE_WRITE_ATTRIBUTES, 0, NULL,
    ...
    if (pft == NULL) {
      ...
      if (SystemTimeToFileTime(&st, &ft) == 0) {
        return printLastError(
            _T("Can't convert system time to file time")); // GCOVR_EXCL_LINE
      }
    }
    if (SetFileTime(hFile, (LPFILETIME)NULL, pft, pft) == 0) {
      return printLastError(
          _T("Can't set access and write time of %s"),
          getReadableFilename(currentPath)); // GCOVR_EXCL_LINE
    }
    if (CloseHandle(hFile) == 0) {
      return printLastError(
          _T("Can't close handle to %s"),
          getReadableFilename(currentPath)); // GCOVR_EXCL_LINE
    }
  }
...
tResult openFile(LPCTSTR currentPath, HANDLE *handle) {
  ...
  *handle = CreateFile(currentPath, GENERIC_READ, FILE_SHARE_READ, NULL,
 ...
 */
tBool isTimeOption(wchar_t *argv) {
  return _tcsncmp(argv, _T("--time"), 6) == 0;
}
...
tResult storeTimeValue(int *argc, wchar_t **argv[]) {
  ...
  SYSTEMTIME lt, st;
  int count = swscanf(localTime, TIME_FORMAT, &lt.wYear, &lt.wMonth, &lt.wDay,
                      &lt.wHour, &lt.wMinute, &lt.wSecond);
  if (count < 3) {
    return printErr(
        _T("Wrong format for time %s, expected yyyy-mm-dd[Thh:mm[:ss]].\n"),
        localTime);
  }
  if (count == 3) {
    lt.wHour = 12;
  ...
  lt.wMilliseconds = 0;
  if (TzSpecificLocalTimeToSystemTime((TIME_ZONE_INFORMATION *)NULL, &lt,
                                      &st) == 0) {
    return printLastError(_T("Can't convert local time %s to system time"),
                          localTime); // GCOVR_EXCL_LINE
  }
 ...
 * @return eOk on success, else eError
 */
tResult runCommandForEachInputLine(int argc, wchar_t *argv[],
                                   tResult (*command)(void)) {
  tResult result = eOk;
      ...
      if (result == eOk) {
        ...
        *IoBufferWideCharacter = _T('\0');
        do {
          ...
          if (FALSE == ReadFile(InHandle, IoBuffer, SIZE_IO_BUFFER - Offset - 1,
                                &BytesRead, NULL)) {
            result &=
                printLastError(_T("Error reading file %s"),
                               DosDevicePathResponseFile); // GCOVR_EXCL_LINE
          }
          if (BytesRead != 0) {
            // Add a \0 after the read content
            IoBuffer[BytesRead] = '\0';
            // ...and convert it.
            if (MultiByteToWideChar(CP_ACP, 0, IoBuffer, -1,
                                    &IoBufferWideCharacter[Offset],
                                    SIZE_IO_BUFFER - Offset) == 0) {
              result &=
                  printLastError(_T("Error converting file content of %s"),
                                 DosDevicePathResponseFile); // GCOVR_EXCL_LINE
            } else {
              ...
              do {
                ...
                if (PtrEndOfLine != NULL) {
                  ...
                  *PtrEndOfLine = _T('\0');
                  do {
                    --PtrEndOfLine;
                    if ((*PtrEndOfLine == _T('\r')) ||
                        (*PtrEndOfLine == _T(' '))) {
                      *PtrEndOfLine = _T('\0');
                    }
                  } while ((*PtrEndOfLine == _T('\0')) &&
                           (PtrEndOfLine > PtrStart));
                  // If line isn't empty
...
tResult checkUniqueName(void) {
  ...
  while (*CurrentFilename != NULL) {
    if (_tcscmp(PtrFilenameStart, *CurrentFilename) == 0) {
      result &=
          printErr(_T("File in source list will overwrite each other: %s"),
                   PtrFilenameStart);
      break;
    }
    ++CurrentFilename;
    ...
    *CurrentFilename = malloc(_tcslen(PtrFilenameStart) * sizeof(TCHAR *));
    if (*CurrentFilename == NULL) {
      result &= printLastError(
          _T("Can't allocate memory for current filenames")); // GCOVR_EXCL_LINE
    }
...
tResult checkUniqueNames(int argc, wchar_t *argv[]) {
  tResult result = eOk;
  size_t ListSize =
      (argc + 1) * sizeof(TCHAR *); // +1 to have a trailing NULL pointer
                                    // (needed for freeing the space)

  KnownFilenames = malloc(ListSize);
  if (KnownFilenames == NULL) {
    result &= printLastError(
        _T("Can't allocate memory for list of filenames")); // GCOVR_EXCL_LINE
  } else {
    ...
    while (*CurrentFilename != NULL) {
      ...
      if (errno != 0) {
        result &= printLastError(
            _T("Can't free memory of filename")); // GCOVR_EXCL_LINE
      }
      ++CurrentFilename;
    }
    free(KnownFilenames);
    if (errno != 0) {
      result &= printLastError(
          _T("Can't free memory of list of filenames")); // GCOVR_EXCL_LINE
    }
  }
...
tResult printFileToHandle(LPCTSTR currentPath, HANDLE *outHandle) {
  ...
  if (result == eOk) {
    ...
    do {
      if (FALSE ==
          ReadFile(InHandle, IoBuffer, SIZE_IO_BUFFER, &BytesRead, NULL)) {
        result &=
            printLastError(_T("Error reading file %s"),
                           getReadableFilename(currentPath)); // GCOVR_EXCL_LINE
      }
      if (BytesRead != 0) {
        ...
        if (WriteFile(outHandle, IoBuffer, BytesRead, &BytesWritten, NULL) ==
            0) {
          result &= printLastError(
              _T("Error writing to output handle")); // GCOVR_EXCL_LINE
        }
      }
    } while ((result == eOk) && (BytesRead != 0));
