...
void errorNoCommand()
{
   print(_T("No command given. Use option --help.\n"));
...
{
   print(_T("Windows file operations\n"));
   print(_T("=======================\n"));
   print(_T("\n"));
   print(_T("Usage:\n"));
   print(_T("  %s [<options>] [<command> [<options>] <argument+>]\n"), ProgramName);
   print(_T("\n"));
   print(_T("The command support paths with up to 32676 characters by using the DOS device\n"));
   print(_T("path e.g. \\\\?\\c:\\\temp. / and \\ are supported as path separator."));
   print(_T("\n"));
   print(_T("To use file or directory names starting with a - or -- add a -- argument.\n"));
   print(_T("The -- argument stops the command line options.\n"));
   print(_T("\n"));
   print(_T("Following options are available:\n"));
   print(_T("   -h, --help           Print this help.\n"));
   print(_T("   -d, --debug          Print additional debug informations.\n"));
   print(_T("\n"));
   print(_T("Following commands are available:\n"));
   print(_T("\n"));
...
int _tmain(int argc, wchar_t *argv[])
{
   ...
   if (ProgramNameLen > PROGRAM_NAME_BUFFER_SIZE)
   {
   ...
   if ((*Command) == NULL)
   {
      printError(_T("Unknown command [%s] given, use option --help for more information.\n"), *argv);
      flushOutputAndExit(EXIT_FAILURE);
   }
   ++argv;
   --argc;

   flushOutputAndExit(((*Command)->run(argc, argv) == VALUE_TRUE) ? EXIT_SUCCESS : EXIT_FAILURE);
