...
tCommand *Commands[] = {&CommandCat,  &CommandCopy,   &CommandMkdir,
                        &CommandMove, &CommandRemove, &CommandTouch,
                        NULL};
...
void printHelpAndExit() {
  ...
  printOut(_T("  %s [<options>] [<command> [<options>] <argument+>]\n"),
           ProgramName);
  printOut(_T("\n"));
  printOut(_T("The command support paths with up to 32676 characters by using ")
           _T("the DOS device\n"));
  printOut(_T("path e.g. \\\\?\\c:\\temp. / and \\ are supported as path ")
           _T("separator."));
  printOut(_T("\n"));
  printOut(_T("To use file or directory names starting with a - or -- add a ")
           _T("-- argument.\n"));
  ...
  printOut(
      _T("   -d, --debug          Print additional debug informations.\n"));
  printOut(_T("\n"));
...
int _tmain(int argc, wchar_t *argv[]) {
  ...
  if (ProgramNameLen > PROGRAM_NAME_BUFFER_SIZE) { // GCOVR_EXCL_BR
    ...
    _tcscat(ProgramName,
            &Ptr[ProgramNameLen - (PROGRAM_NAME_BUFFER_SIZE + 3 + 1)]);
    // GCOVR_EXCL_STOP
  } else {
  ...
  while (1 == 1) {
    if ((_tcscmp(*argv, _T("--help")) == 0) ||
        (_tcscmp(*argv, _T("-h")) == 0)) {
      printHelpAndExit();
    } else if ((_tcscmp(*argv, _T("--debug")) == 0) ||
               (_tcscmp(*argv, _T("-d")) == 0)) {
      Debug = 1;
      ++argv;
      --argc;
  ...
  if ((*Command) == NULL) {
    printErr(_T("Unknown command '%s' given, use option --help for more ")
             _T("information.\n"),
             *argv);
    flushOutputAndExit(EXIT_FAILURE);
  }
  ...
  flushOutputAndExit(((*Command)->run(argc, argv) == eOk) ? EXIT_SUCCESS
                                                          : EXIT_FAILURE);
