      ...
      if (isDirectory(dwAttrs)) {
         ...
         if (Recursive && (result == eTrue)) {
            ...
            if (hFind == INVALID_HANDLE_VALUE) {
               ...
               } while ((result == eTrue) && (FindNextFile(hFind, &FindFileData) != 0));
               if (result == eTrue) {
         ...
         if (result == eTrue) {
            ...
            if (Touch) {
               result &= touchSingleFile(TargetDosDevicePath, LocalTime, eFalse);
            }
         }
      }
   }
