...
{
   print(_T("Copy files or directories to STDOUT.\n"));
   print(_T("\n"));
   print(_T("Available options are (Arguments for long options are also needed for short\n"));
   print(_T("options):\n"));
   print(_T("   -r, -R, --recursive  Copy files recursive.\n"));
   ...
   print(_T("                        given, the last argument is used as target directory.\n"));
...
static tBool copyOperation(void)
{
   ...
   if (result == VALUE_TRUE)
   {
      ...
      if (isDirectory(dwAttrs))
      {
         ...
         if (Recursive && (result == VALUE_TRUE))
         {
...
static tBool runCommand(int argc, wchar_t *argv[])
{
   ...
   if ( result == VALUE_TRUE ) {
      result &= runCommandForEachInputLine(argc, argv, copyOperation);
   }
