...
static void printHelp(void) {
  ...
  printOut(_T("\n"));
  printOut(_T("Available options are (Arguments for long options are also ")
           _T("needed for short\n"));
  printOut(_T("options):\n"));
  printOut(_T("   -f, --force          Ignore write protection of existing ")
           _T("targets.\n"));
  printOut(_T("   --touch              Touch the files after copy.\n"));
  printOut(_T("   --time=TIME          Use local ISO time instead of current ")
           _T("system time for.\n"));
  printOut(_T("                        the touch operation, e.g. ")
           _T("2021-01-31T15:05:01. If the\n"));
  printOut(
      _T("                        time is omitted, 12:00:00 is assumed.\n"));
  printOut(
      _T("   -t, --target-directory=DIR  Target directory to use. If not\n"));
  printOut(_T("                        given, the last argument is used as ")
           _T("target dir.\n"));
  printOut(_T("   --check-unique-names If multiple files or folders are ")
           _T("copied check upfront\n"));
  printOut(_T("                        if there are any collisions. This ")
           _T("isn't supported with\n"));
  printOut(_T("                        wildcards.\n"));
  printOut(_T("\n"));
  printOut(_T("The write protection is not copied to the target elements.\n"));
  printOut(_T("If several sources are given or target is an existing ")
           _T("directory the files\n"));
  ...
  if (result == eOk) {
    ...
    if (isDirectory(dwAttrs)) {
      if (RecursiveMoveNeeded == eFalse) {
        if (Debug) {
          printOut(_T("Try to move %s directly to %s, this only works on same ")
                   _T("device.\n"),
      ...
      if (RecursiveMoveNeeded == eTrue) {
        ...
        if (isDirectory(dwAttrsTarget) == 0) {
          result &=
              createSingleDirectory(TargetDosDevicePath); // GCOVR_EXCL_LINE
        }
        if (result == eOk) {
...
static tResult runCommand(int argc, wchar_t *argv[]) {
  ...
  while (argc != 0) {
    if ((_tcscmp(*argv, _T("--help")) == 0) ||
      ...
      printHelp();
      return eOk;
    } else if ((_tcscmp(*argv, _T("--force")) == 0) ||
               (_tcscmp(*argv, _T("-f")) == 0)) {
      Force = eTrue;
    ...
    } else if ((_tcscmp(*argv, _T("-t")) == 0) ||
               (_tcsncmp(*argv, _T("--target-directory"), 18) == 0)) {
      LPCTSTR Ptr;
      ...
      --argc;
      break;
    } else if ((_tcsncmp(*argv, _T("--"), 2) == 0) ||
               (_tcsncmp(*argv, _T("-"), 1) == 0)) {
      return printErr(
          _T("Unknown option %s, use option --help for more information.\n"),
          *argv);
    } else {
      break;
    }
  ...
  if (TargetIsDirectory) {
    ...
    if (isValidFileAttributes(dwAttrsTarget)) {
      if (isDirectory(dwAttrsTarget) == 0) {
        return printErr(_T("Target %s must be a directory."),
                        getReadableFilename(TargetDosDevicePath));
      ...
      return printErr(_T("Directory %s doesn't exist."),
                      getReadableFilename(TargetDosDevicePath));
    }
  }
