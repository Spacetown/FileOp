...
static void printHelp(void) {
  printOut(_T("Touch the given files. This means creating it or updating the ")
           _T("timestamp if it\n"));
  printOut(_T("already exists.\n"));
  printOut(_T("If the file starts with an @, the files listed in the files ")
           _T("are touched.\n"));
  printOut(_T("\n"));
  printOut(_T("Available options are:\n"));
  printOut(_T("   --time=TIME  Use local ISO time instead of current system ")
           _T("time.\n"));
  printOut(_T("                E.g. 2021-01-31T15:05:01 if no time is given, ")
           _T("12:00:00 is\n"));
  printOut(_T("                assumed.\n"));
...
static tResult touchOperation(void) {
  return touchSingleFile(DosDevicePath, eTrue);
}
